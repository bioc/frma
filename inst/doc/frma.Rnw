...
\subsubsection{Multiple Arrays: Batch Effect Adjusted}
If you want the batch effect adjustment method described in the fRMA
paper, the quickest way of reading in data and getting expression
measures is the following:
\begin{enumerate}
\item Download and install frma and the appropriate data file (i.e. hgu133afrmavecs, hgu133plus2frmavecs, etc.).
...
library(frma)
@ 
\item Read in the data and preprocess using the ``batch'' option.
<<echo=T,results=hide>>=
library(frmaExampleData)
data(AffyBatchExample)
...
In each of the cases above, the final \Robject{object} will be an
ExpressionSet. To obtain a matrix of gene-level expression values,
...
\begin{itemize}
  ...
  \item {\bf{median polish}:} subtract the probe-effect and then compute the median of the probes in each probeset
  ...
  \item {\bf{robust weighted average}:} compute a weighted average of the probes in each probeset with weights equal to the weights returned by an M-estimation procedure divided by the sum of the precomputed within and between batch variance estimates. 
  \item {\bf{batch}:} the robust weighted average method adapted for a batch of new arrays.
\end{itemize}
...
estimates, the frma function can also return additional information
about the estimates depending on the summarization method chosen. A
description of the arguments that can be included in the output.param
argument follows:
\begin{itemize}
  \item {\bf{stderr}:} the standard errors of the gene-level expression estimates.
...
methods; however, all three are available for the default summarization procedure -- robust weighted average.
